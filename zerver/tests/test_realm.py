...
class RealmAPITest(ZulipTestCase):
    ...
    def update_with_realm_default_api(self, name: str, val: Any) -> None:
        ...
        result = self.client_patch("/json/realm_default", {name: val})
        self.assert_json_success(result)
    ...
    def do_test_realm_default_setting_update_api(self, name: str) -> None:
        ...
        test_values: Dict[str, Any] = dict(
            ...
            emojiset=[emojiset["key"] for emojiset in UserProfile.emojiset_choices()],
        ...
        vals = test_values.get(name)
        if name in RealmUserDefault.property_types:
            property_type = RealmUserDefault.property_types[name]
        elif name != "enter_sends":
            property_type = RealmUserDefault.notification_setting_types[name]

        if name == "enter_sends" or property_type is bool:
            vals = bool_tests
    ...
    def test_update_default_realm_settings(self) -> None:
        ...
        for prop in RealmUserDefault.notification_setting_types:
            self.do_test_realm_default_setting_update_api(prop)
        self.do_test_realm_default_setting_update_api("enter_sends")

    def test_invalid_default_notification_sound_value(self) -> None:
        result = self.client_patch("/json/realm_default", {"notification_sound": "invalid"})
        self.assert_json_error(result, "Invalid notification sound 'invalid'")

        result = self.client_patch("/json/realm_default", {"notification_sound": "zulip"})
        self.assert_json_success(result)
        realm = get_realm("zulip")
        realm_user_default = RealmUserDefault.objects.get(realm=realm)
    ...
    def test_ignored_parameters_in_realm_default_endpoint(self) -> None:
        ...
        json_result = self.client_patch("/json/realm_default", params)
