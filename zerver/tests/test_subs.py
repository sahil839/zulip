...
class TestCreateStreams(ZulipTestCase):
    ...
    def test_set_stream_admin_on_stream_creation(self) -> None:
        ...
        result = self.common_subscribe_to_streams(hamlet, [stream_public, stream_private],
                                                  {"principals": orjson.dumps([hamlet.id,
                                                                               cordelia.id]).decode()})
        self.assert_json_success(result)
...
class StreamAdminTest(ZulipTestCase):
    ...
    def test_stream_admin_remove_multiple_users_from_stream(self) -> None:
        ...
        target_users = [
            self.example_user(name)
            for name in ['cordelia', 'prospero', 'othello', 'ZOE', 'aaron']
        ]
        result = self.attempt_unsubscribe_of_principal(
            query_count=34,
            cache_count=9,
    ...
    def test_change_subscription_role_for_public_stream(self) -> None:
        ...
        def change_subscription_role() -> None:
            for role in [Subscription.ROLE_STREAM_ADMINISTRATOR, Subscription.ROLE_MEMBER]:
                result = self.client_patch(f"/json/users/{othello.id}/subscriptions/{stream.id}",
        ...
        cordelia = self.example_user('cordelia')
        result = self.client_patch(f"/json/users/{cordelia.id}/subscriptions/{stream.id}",
    ...
    def test_change_subscription_role_for_private_stream(self) -> None:
        ...
        self.assert_json_error(result, "Organization administrators cannot change role for unsubscribed private stream.")
