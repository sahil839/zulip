...
@has_request_variables
def update_subscription_role(request: HttpRequest, user_profile: UserProfile,
                             user_id: int=REQ(validator=check_int, path_only=True),
                             stream_id: int=REQ(validator=check_int, path_only=True),
                             role: int=REQ(validator=check_int_in(Subscription.ROLE_TYPES),)
                             ) -> HttpResponse:
    (stream, sub) = access_stream_for_delete_or_update(user_profile, stream_id)
    ...
    if stream.invite_only and sub is None:
        raise JsonableError(_("Organization administrators cannot change role for unsubscribed private stream."))

    try:
        target_user_sub = Subscription.objects.get(user_profile=target_user,
