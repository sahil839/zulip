...
def copy_default_settings(
    ...
    for settings_name in UserBaseSettings.notification_setting_types:
        value = getattr(settings_source, settings_name)
        setattr(target_profile, settings_name, value)

    setattr(target_profile, "enter_sends", settings_source.enter_sends)
...
def create_user(
    ...
    elif realm.string_id != settings.SYSTEM_BOT_REALM:
        realm_user_default = RealmUserDefault.objects.get(realm=realm)
        copy_default_settings(realm_user_default, user_profile)
    else:  # nocoverage
        # This will be executed only for internal realm.
        user_profile.save()
